            ...
            <?php if (!empty($cartItems)) : ?>
              <?php foreach ($cartItems as $item) : ?>
                <tr>
                  ...
                  <td>
                    <a href="cart.php?do=remove-product&id=<?php echo $item['id']; ?>" class="btn btn-danger"><i class="fa-solid fa-trash"></i>&nbsp;Xóa</a>
                  </td>
                </tr>
              <?php endforeach; ?>
            <?php else : ?>
              <tr>
                <td colspan="5">Giỏ hàng của bạn trống.<br /><a class="btn btn-outline-dark" href="./index.php">Trở về trang chủ</a></td>
              </tr>
            <?php endif ?>
          </tbody>
          <tfoot>
        ...
        <div class="row g-3">
          <div class="col-md-6">
            ...
            <div class="form-group">
              <label for="name_customer">Họ và tên *</label>
          ...
          <div class="col-md-6">
            ...
            <button type="submit" name="place_order" class="btn btn-primary" disabled><i class="fa-solid fa-check-double"></i>&nbsp;Đặt hàng</button>
          </div>
        </div>
      </form>
    </div>
...
?>
  <div class="order-received">
    <div class="container">
      <h1>Cảm ơn. Đơn hàng của bạn đã được nhận.</h1>
        ...
        ?>
          <div class="container">
            <div class="alert alert-warning text-center mt-5" role="alert">
              Không tìm thấy đơn hàng nào cho khách hàng này.
            </div>
          </div>
      <?php
        }
...
<?php
  ...
  if (isset($_POST['add_to_cart'])) {
    ...
    show_message('Thêm ' . $product_name . ' vào giỏ hàng thành công.', 'success');
    header('location: ' . $_SERVER['HTTP_REFERER']);
    exit();
  } else {
  ...
  if (isset($_GET['id'])) {
    ...
    show_message('Xóa ' . $products . ' khỏi giỏ hàng thành công.', 'success');
    header('location: ' . $_SERVER['HTTP_REFERER']);
    exit();
  } else {
    header('location: index.php');
